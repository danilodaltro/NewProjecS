...
namespace ProjetoS.Application.Service.PessoaService
{
    public class CidadeService : BaseService<CidadeService>, ICidadeService
    {
        ...
        {
            var entity = await _db.Cidade.FirstOrDefaultAsync(c => c.Id == id);

            var pessoaEntity = await _db.Pessoa.FirstOrDefaultAsync(p => p.IdCidade == id);

            if (pessoaEntity != null)
                return new DeleteCidadeResponse()
                {
                    Success = false,
                    Error = "A cidade não pode ser excluída enquanto houver pessoas relacioadas."
                };
